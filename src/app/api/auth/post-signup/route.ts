...
export async function POST(req: Request) {
  ...
  try {
    ...
    // US001-AC06: Start 3-month free access period
    const freeAccessExpiresAt = new Date();
    freeAccessExpiresAt.setMonth(freeAccessExpiresAt.getMonth() + 3);

    // Only update if it hasn't been set before to avoid overwriting
