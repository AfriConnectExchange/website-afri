...
export const AdvertExpiredEmail = ({
  sellerName = 'Seller',
  advertTitle = 'Your Advert',
  expiryDate = new Date().toLocaleDateString(),
  renewUrl = '#',
  homeUrl = '#',
  supportEmail = 'support@africonnect-exchange.org',
  appName = 'AfriConnect Exchange'
}: AdvertExpiredEmailProps) => {
  const year = new Date().getFullYear();
  const previewText = `Your advert "${advertTitle}" has expired`;
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td style={{ padding: "28px 28px 8px 28px" }}>
                          ...
                          <Text style={text}>Your advert, "{advertTitle}", has reached its expiration date and is no longer visible to customers.</Text>
                          <Section style={advertBox}>
                            <Text style={advertTitleStyle}>"{advertTitle}"</Text>
                            <Text style={expiryText}>Expired on: {expiryDate}</Text>
                          </Section>
                          <Text style={text}>Want to keep reaching more customers? Renew your advert now!</Text>
...
const h1 = { margin: '0 0 16px', fontFamily: "'Montserrat', Arial, sans-serif", fontSize: '22px', color: '#EA4335', lineHeight: 1.2 };
const text = { margin: '0 0 16px', fontSize: '15px', color: '#374151', lineHeight: 1.6 };
const advertBox = { backgroundColor: '#FFFBEB', borderRadius: '8px', padding: '20px', margin: '24px 0', textAlign: 'center' as const, border: '1px solid #FDE68A' };
const advertTitleStyle = { color: '#2C2A4A', fontSize: '18px', fontWeight: 'bold', margin: '0 0 8px 0' };
const expiryText = { color: '#B45309', fontSize: '14px', margin: '0' };
const buttonContainer = { textAlign: 'center' as const, margin: '24px 0' };
const button = { backgroundColor: '#F4B400', borderRadius: '6px', color: '#ffffff', padding: '12px 24px', fontSize: '14px', fontWeight: 'bold', textDecoration: 'none' };
